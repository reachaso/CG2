...
#include <DbgHelp.h>
#include <strsafe.h>
#pragma comment(lib, "Dbghelp.lib") //DbgHelp.libをリンクする
...
static LONG WINAPI ExportDump(PEXCEPTION_POINTERS exception) {
  // 時刻を取得して、時刻を名前に入れたファイルを作成。Dumpsのディレクトリ以下に出力
  SYSTEMTIME time;
  GetLocalTime(&time);
  ...
  // Fumpを出力。MiniDumpNormalは最低限の情報を出力するフラグ
  MiniDumpWriteDump(GetCurrentProcess(), processId, dumpFileHandle,
                    MiniDumpNormal, &minidumpInformation, nullptr, nullptr);
  // 他に関連づけられているSSH例外ハンドラがあれば実行。通常はプロセスを終了する
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	// コマンドキューの生成
        ...
        rtvDesc.Format =
            DXGI_FORMAT_R8G8B8A8_UNORM_SRGB; // 出力結果をSRGBに変換する
        ...
        device->CreateRenderTargetView(swapChainResource[0], &rtvDesc,
                                       rtvHandles[0]);
        // 二つ目を作る
        rtvHandles[1].ptr =
            rtvHandles[0].ptr + device->GetDescriptorHandleIncrementSize(
                                    D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        device->CreateRenderTargetView(swapChainResource[1], &rtvDesc,
                                       rtvHandles[1]);
	...
	while (msg.message != WM_QUIT) {
        ...
        commandList->OMSetRenderTargets(1, &rtvHandles[backBufferIndex], false, nullptr);
                        ...
                        commandList->ClearRenderTargetView(
                            rtvHandles[backBufferIndex], clearColor, 0,
                            nullptr);
